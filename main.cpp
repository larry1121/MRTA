...
int main()
{
    ...
    // Calculate task discovery statistics
    double sum = accumulate(discovered_tasks_per_run.begin(), discovered_tasks_per_run.end(), 0.0);
    double average = sum / NUM_RUNS;

    int min_tasks = *min_element(discovered_tasks_per_run.begin(), discovered_tasks_per_run.end());
    int max_tasks = *max_element(discovered_tasks_per_run.begin(), discovered_tasks_per_run.end());

    // Count how many runs found all 16 tasks
    int perfect_runs = count(discovered_tasks_per_run.begin(), discovered_tasks_per_run.end(), NUM_MAX_TASKS);

    // Calculate drone energy statistics
    int num_drones = drone_energy_per_run.empty() ? 0 : static_cast<int>(drone_energy_per_run[0].size());
    vector<double> avg_drone_energy(num_drones, 0.0);
    vector<int> min_drone_energy(num_drones, INT_MAX);
    vector<int> max_drone_energy(num_drones, 0);
    ...
    if (num_drones > 0)
    {
        for (int drone_idx = 0; drone_idx < num_drones; ++drone_idx)
        {
            ...
            for (int run = 0; run < NUM_RUNS; ++run)
            {
                ...
                total_energy += energy;
                min_drone_energy[drone_idx] = min(min_drone_energy[drone_idx], energy);
                max_drone_energy[drone_idx] = max(max_drone_energy[drone_idx], energy);
            }
    ...
    if (num_drones > 0)
    {
        ...
        for (int drone_idx = 0; drone_idx < num_drones; ++drone_idx)
        {
            ...
            cout << "  Average remaining energy: " << fixed << setprecision(1) << avg_drone_energy[drone_idx] << endl;
            cout << "  Min remaining energy: " << min_drone_energy[drone_idx] << endl;
            cout << "  Max remaining energy: " << max_drone_energy[drone_idx] << endl;
            cout << "  Average energy used: " << fixed << setprecision(1)
                 << (ROBOT_ENERGY - avg_drone_energy[drone_idx]) << endl;
            cout << "  Energy usage rate: " << fixed << setprecision(1)
        ...
        cout << "Average remaining energy (all drones): " << fixed << setprecision(1) << overall_avg_energy << endl;
        cout << "Average energy used (all drones): " << fixed << setprecision(1)
             << (ROBOT_ENERGY - overall_avg_energy) << endl;
        cout << "Overall energy usage rate: " << fixed << setprecision(1)
