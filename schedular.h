...
private:
    ...
    Coord action_to_delta(ROBOT::ACTION action)
    {
        switch (action)
        {
        case ROBOT::ACTION::UP:
            return {0, 1};
        case ROBOT::ACTION::DOWN:
            return {0, -1};
        case ROBOT::ACTION::LEFT:
            return {-1, 0};
        case ROBOT::ACTION::RIGHT:
            return {1, 0};
        case ROBOT::ACTION::HOLD:
            return {0, 0};
        default:
    ...
    static inline int default_unknown_cost(ROBOT::TYPE t)
    {
        switch (t)
        {
        case ROBOT::TYPE::CATERPILLAR:
            return UNKNOWN_COST_CATERPILLAR;
        case ROBOT::TYPE::WHEEL:
            return UNKNOWN_COST_WHEEL;
        default:
            return INFINITE / 4; // 드론은 안 씀
        }
    }
                                          ...
                                          const vector<vector<vector<int>>> &cost_map)
    {
        ...
        if (cost1 == -1)
            cost1 = default_unknown_cost(r_type);
        if (cost2 == -1)
            cost2 = default_unknown_cost(r_type);
