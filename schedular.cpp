                        ...
                        std::vector<Coord> &out_path_coords)
{
    ...
    while (!pq.empty())
    {
        ...
        // Early termination if cost exceeds robot's energy
        if (current_cost_pq >= robot.get_energy())
        {
            return std::numeric_limits<int>::max();
        }
...
void Scheduler::set_hyperparameters(int cluster_dist, int energy_margin, int max_cluster_size, int drone_pause, int drone_resume)
{
    CLUSTER_DISTANCE_THRESHOLD = cluster_dist;
    ENERGY_MARGIN_PERCENT = energy_margin;
    MAX_CLUSTER_SIZE = max_cluster_size;
    DRONE_exploration_pause_start = drone_pause;
